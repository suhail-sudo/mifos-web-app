    ...
    mifosX.services = _.extend(module, {
        AuthenticationService: function (scope, httpService, SECURITY, localStorageService,timeout, webStorage) {
            ...
            this.authenticateWithUsernamePassword = function (credentials) {
        		...
        		/*if(SECURITY === 'oauth'){
	                httpService.post( "/fineract-provider/api/oauth/token?username=" + credentials.username + "&password=" +
                        credentials.password +"&client_id=community-app&grant_type=password&client_secret=123")
                ...
                if(SECURITY === 'oauth'){
                    ...
                    httpService.post(apiVer + '/authentication?tenantIdentifier=default', credentials)
                        .success(onLoginSuccess)
                        .error(onLoginFailure);
                }
            };
