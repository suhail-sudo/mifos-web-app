(function (module) {
    mifosX.models = _.extend(module, {
        ClientStatus: function () {
            ...
            this.getStatus = function (status, isAgent) {
            	var statusTypes = [];
            	if(isAgent) {
            		statusTypes = this.statusTypes[status];
            		for(var i=0; i<statusTypes.length; i++) {
            			if(statusTypes[i].href == '#/editclient')
            				statusTypes[i].href = '#/editagent'
            			if(statusTypes[i].name == "label.button.transferclient")
            				statusTypes[i].name = "label.button.transferagent";
            		}
            	} else
            		statusTypes = this.statusTypes[status];
            	return statusTypes;
            ...
            this.statusTypes = {
                "Pending": [
                    {
                        name: "label.button.facility",
                        href: "#/newLineFacilityContract",
                        icon: "fa fa-plus",
