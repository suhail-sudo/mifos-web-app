(function (module) {
    mifosX.controllers = _.extend(module, {
        CreateLineFacilityProductController: function (scope, $rootScope, resourceFactory, location, dateFilter,WizardHandler) {
            ...
            scope.cycleSelect = angular.element('#repaymentCycle option:selected').text();
            scope.selectedCycle = function(){
                scope.cycleSelect = angular.element('#repaymentCycle option:selected').text();
            };
            scope.limitSelect = angular.element('#limitAvailability option:selected').text();
            scope.selectedLimit = function(){
                scope.limitSelect = angular.element('#limitAvailability option:selected').text();
            };
            scope.minimumPaySel = angular.element('#minimumPay option:selected').text();
            scope.selectedPayment = function(){
                scope.minimumPaySel = angular.element('#minimumPay option:selected').text();
            };

            scope.herrPaySel = angular.element('#transactionProcessingStrategyId option:selected').text();;
            scope.selectedHerr = function(){
                scope.herrPaySel = angular.element('#transactionProcessingStrategyId option:selected').text();
            };
            scope.breaPaySel = angular.element('#repaymentBreach option:selected').text();
            scope.selectedBreach = function(){
                scope.breaPaySel = angular.element('#repaymentBreach option:selected').text();
            };
            ...
            scope.cancel = function () {
                location.path('/lineFacilityProducts');
            };
            ...
            */
            ...
            scope.submit = function () {
             ...
             if(!this.formData.allowVariableInstallments) {
                ...
                resourceFactory.getLineFacilityProductsResource.saveLineFacilityProduct(this.formData, function (data) {
                    console.log(data);
                    location.path('/lineFacilityProducts/');
                });
            };
    ...
    mifosX.ng.application.controller('CreateLineFacilityProductController', ['$scope','$rootScope', 'ResourceFactory', '$location', 'dateFilter','WizardHandler', mifosX.controllers.CreateLineFacilityProductController]).run(function ($log) {
        $log.info("CreateLineFacilityProductController initialized");
    });
}(mifosX.controllers || {}));
