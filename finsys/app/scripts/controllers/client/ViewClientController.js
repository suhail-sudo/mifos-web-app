(function (module) {
    mifosX.controllers = _.extend(module, {
        ViewClientController: function (scope, routeParams, route, location, resourceFactory, http, $uibModal, API_VERSION, $rootScope, Upload, $filter, dateFilter,localStorageService) {
            scope.trs = [];
            scope.client = [];
           ...
           // scope.families=[];
            ...
            scope.showsummary = function () {
            	scope.transactionSummary = [];
                var formatedDate =  dateFilter(scope.summary.summarydate,  "MM/dd/yyyy"); 
                resourceFactory.consumeLimitSummaryResource.getAll({"entityName" : "Client", "entityId" :routeParams.id, "date" : formatedDate},function(limits){
                resourceFactory.ruleMasterResource.getAll({"category" : "master"}, function (data) {
                    ...
                    _.each(limits, function(limit){
                        ...
                        limit.lastModifiedTime = $filter('DateFormat')(limit.lastModifiedTime, scope.df); 
                        scope.transactionSummary.push(limit);
                    });
                });
            });
           }
            ...
            scope.routeToViewFacilityContract = function (id) {
                localStorageService.addToLocalStorage('clientId',scope.client.id);
                location.path('/viewLineFacilityContract/'+ id);
            };
            ...
            resourceFactory.clientResource.get({clientId: routeParams.id}, function (data) {
                ...
                scope.buttonsArray = {
                    options: [
                        {
                            name: scope.isAgent?"button.agentscreenreports": "button.clientscreenreports"
                        }
                    ]
                };
            ...
            resourceFactory.lineFacilityContractResource.getContracts({clientId: routeParams.id},function (data) {
                console.log(data);
                scope.lineContracts = data;
                scope.facilitylength = scope.lineContracts.length;
            });
            ...
            scope.deleteDocument = function (documentId, index) {
                swal({
                    title: 'Are you sure you want to delete client document?',
    ...
    mifosX.ng.application.controller('ViewClientController', ['$scope', '$routeParams', '$route', '$location', 'ResourceFactory', '$http', '$uibModal', 'API_VERSION', '$rootScope', 'Upload', '$filter', 'dateFilter','localStorageService', mifosX.controllers.ViewClientController]).run(function ($log) {
