        ...
        SearchTransactionController: function (scope, resourceFactory, paginatorService, dateFilter, location) {
            ...
            var fetchFunction = function (offset, limit, callback) {
                ...
                if(scope.formData.savingsaccountId){
                  ...
                  if(scope.formData.facilityId){
                    params.facilityId = scope.formData.facilityId;
                    scope.searchCriteria.journals[8] = params.facilityId;
                } else
                    scope.searchCriteria.journals[8] = null;    
            ...
            if(location.search().savingsId != null){
               /* alert(location.search().savingsId);
                alert(location.search().facilityId);*/
                scope.formData.savingsaccountId = location.search().savingsId;
                scope.formData.facilityId=location.search().facilityId
                scope.displayResults = true;
                scope.transactions = paginatorService.paginate(fetchFunction, 14);
                scope.isCollapsed = false;
                scope.isValid = true;
