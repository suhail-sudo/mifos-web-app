    ...
    mifosX.controllers = _.extend(module, {
        ViewSavingDetailsController: function (scope, routeParams, resourceFactory, paginatorService, location, $uibModal, route, dateFilter, $sce, $rootScope, API_VERSION) {
            ...
            scope.clickEvent = function (eventName, accountId) {
                ...
                switch (eventName) {
                    ...
                    case "postInterestAsOn":
                        ...
                        break;
                    case "updateSavingsProduct":
                        location.path('/updatesavingsproduct/' + accountId  );
                        break;
            ...
            resourceFactory.savingsResource.get({accountId: routeParams.id, associations: 'all'}, function (data) {
                ...
                scope.facilityObj=null;
                //Added by tope
                 resourceFactory.savingsAccountsLineFacility.get({savingsAccount: scope.savingaccountdetails.accountNo }, function (data2) {
                   scope.facilityObj=data2;
                    scope.savingaccountdetails.facilityId=data2.facilityId;
                });
                ...
                if (data.status.value == "Approved") {
                    scope.buttons = { singlebuttons: [
                        ...
                        {
                            name: "button.updateSavingsProduct",
                            taskPermissionName:"UPDATE_SAVINGSACCOUNT"
                        }
                    ]
                    };
            ...
            scope.export = function () {
               ...
               scope.savingsLineFacilityObj = null;
                
              scope.viewJournalEntries = function(){
                resourceFactory.savingsAccountsLineFacility.get({savingsAccount: scope.savingaccountdetails.accountNo }, function (data) {
                    scope.savingsLineFacilityObj = data;
                    location.path("/searchtransaction/").search({savingsId: scope.savingaccountdetails.id,facilityId: scope.savingsLineFacilityObj.facilityId});
                });
                //alert(scope.savingsLineFacilityObj.faclityId);
               // 
            };
                
             scope.viewJournalEntries2 = function(){
               
                location.path("/searchtransaction/").search({savingsId: scope.savingaccountdetails.id,facilityId: scope.savingsLineFacilityOb.facilityId});
            };
            ...
            scope.viewSavingsTransactionReceipts = function (transaction) {
                ...
                scope.hidePentahoReport = true;
                scope.formData.outputType = 'PDF';
                scope.baseURL = $rootScope.hostUrl + API_VERSION + "/runreports/" + encodeURIComponent("Transaction Receipt");
                scope.baseURL += "?output-type=" + encodeURIComponent(scope.formData.outputType) + "&tenantIdentifier=" + $rootScope.tenantIdentifier+"&locale="+scope.optlang.code;

                var reportParams = "";

                reportParams += encodeURIComponent("R_refNo") + "=" + encodeURIComponent(transaction.tranRefNo);
                reportParams += "&" + encodeURIComponent("R_clientId") + "=" + encodeURIComponent(scope.savingaccountdetails.clientId);
                if (reportParams > "") {
                    scope.baseURL += "&" + reportParams;
                }
                // allow untrusted urls for iframe http://docs.angularjs.org/error/$sce/insecurl
