(function (module) {
    mifosX.controllers = _.extend(module, {
        OfficesController: function (scope, resourceFactory, location, $routeParams) {
            scope.offices = [];
            scope.isTreeView = false;
            var idToNodeMap = {};
			scope.isOutlet = $routeParams.isOutlet == "true";
            scope.routeTo = function (id) {
                location.path('/viewoffice/' + id).search('isOutlet', scope.isOutlet);
            };
            ...
            resourceFactory.officeResource.getAllOffices({isOutlet: $routeParams.isOutlet}, function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (typeof currentObj.parentId === "undefined" || idToNodeMap[currentObj.parentId] === "undefined") {
    ...
    mifosX.ng.application.controller('OfficesController', ['$scope', 'ResourceFactory', '$location', '$routeParams', mifosX.controllers.OfficesController]).run(function ($log) {
