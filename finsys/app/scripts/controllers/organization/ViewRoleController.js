            ...
            resourceFactory.rolePermissionResource.get({roleId: routeParams.id}, function (data) {
                ...
                for (var i in data.permissionUsageData) {
                    if (data.permissionUsageData[i].grouping != currentGrouping) {
                        ...
                        tempPermissionEtyUIData[currentGrouping] = { entities: {} };
                    }
                    ...
                    var entityName = data.permissionUsageData[i].entityName;
                	if(!data.permissionUsageData[i].entityName) entityName = 'special';
                    if(data.permissionUsageData[i].entityName == currentEntity){
                    	tempPermissionEtyUIData[currentGrouping].entities[entityName].permissions.push(temp);
                    	
                    }else{
                    	currentEntity = data.permissionUsageData[i].entityName;
                    	tempPermissionEtyUIData[currentGrouping].entities[entityName] =  { permissions: [] };
                    	tempPermissionEtyUIData[currentGrouping].entities[entityName].permissions.push(temp);
                    }
                }
                ...
                {//backup -> save the data from formData before editing to boolean array
                    for(var i = 0; i< this.permissions.permissions.length; i++)
                    {
                        ...
                        var temp1 = this.formData[this.permissions.permissions[i].entityName];
                        bValuesOnly.push(temp1);
                ...
                scope.showEntities = function (grouping) {
                    if (scope.previousGrouping) {
                    	tempPermissionEtyUIData[scope.previousGrouping].entities = scope.entities;
                    }
                    scope.entities = tempPermissionEtyUIData[grouping].entities ;
                    scope.previousGrouping = grouping;
                    scope.showPermissions(Object.keys(scope.entities)[0]);
                };

                scope.showPermissions = function (entity) {
                    scope.permissions = tempPermissionEtyUIData[scope.previousGrouping].entities[entity];
                    scope.previousEntity = entity;
                };
                //by default show special permissions
                scope.showEntities('special');
